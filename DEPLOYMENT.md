...
`pnpm run preflight:cutover`), so the DNS edits below are made in the Cloudflare
...
`pnpm run check:basepath` gate runs in the deploy right after the build and
...
```bash
pnpm run preflight:cutover          # expect: "CUTOVER COMPLETE"
pnpm run smoke -- https://catnipandcattitude.org
```
